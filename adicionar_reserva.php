        ...
        <div class="card-ios">
            ...
            <div class="mb-3">
                ...
                <div style="display:flex; gap:10px; margin-top:10px;">
                    <button type="button" onclick="importarWhatsParaFormulario()" class="btn-ios-outline">Importar p/
                        Form</button>
                    <button type="button" onclick="analisarSalvarDireto()" class="btn-ios-secondary">Salvar
                        Direto</button>
                </div>
            </div>
            <hr>
            ...
            <form id="formManual" onsubmit="event.preventDefault(); verificarEEnviar();">
                ...
                <div id="card-perfil-cliente" class="profile-card">
                    <div class="profile-header">
                        <div class="profile-info-area">
                            <div class="avatar-circle"><svg viewBox="0 0 24 24">
                                    <path
                            ...
                            <div class="client-details">
                                ...
                                <p style="font-size:0.85rem; margin-top:5px;">
                                    Última visita: <span id="card-ultima" style="font-weight:bold;">--</span> (<span
                                        id="card-tempo" style="color:#007AFF;">--</span>)<br>
                                    <span style="color:#666; font-size:0.8rem;">Histórico (4 últ.): <span
                                            id="card-historico">--</span></span>
                                </p>
                            </div>
                        </div>
                        <div class="profile-actions">
                            <button type="button" class="btn-profile-action" onclick="trocarCliente()">Trocar</button>
                            <button type="button" class="btn-profile-action" id="btn-editar-cliente"
                                onclick="editarCliente()">Editar Nome</button>
                        </div>
                    </div>
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-box"><small>Canceladas</small><strong id="stat-cancelada"
                                class="alert-cancel">0</strong></div>
                    </div>
                </div>

                <div class="mb-2" id="div-input-nome"> <label>Nome:</label> <input type="text" id="nome" name="nome"
                        class="form-control" required> </div>

                <div class="row">
                    <div class="col-md-6 mb-2"> <label>Data:</label> <input type="date" id="data" name="data"
                            class="form-control" required> </div>
                    <div class="col-md-6 mb-2"> <label>Horário:</label> <input type="time" id="horario" name="horario"
                            class="form-control" required> </div>
                </div>

                <div class="mb-2"> <label>Nº de Pessoas:</label> <input type="number" id="num_pessoas"
                        name="num_pessoas" class="form-control" required> </div>
                <div class="mb-2"> <label>Telefone Alt:</label> <input type="text" id="telefone2" name="telefone2"
                        class="form-control" onkeyup="maskPhone(this)"> </div>
                ...
                <div class="mb-2"> <label>Observações:</label> <textarea id="observacoes" name="observacoes"
    ...
    <script>
        ...
        function toggleTelefone(checkbox) {
            ...
            if (checkbox.checked) {
                ...
                telInput.value = '';
                telInput.disabled = true; // Impede envio de dados sujos
                telInput.style.backgroundColor = '#e9ecef';

                // Força exibição do nome manual, pois não há busca de perfil
        ...
        function fecharModalEAtualizar() {
            ...
            // Limpeza completa do formulário sem recarregar a página
            document.getElementById('formManual').reset();
            document.getElementById('whats_dados').value = '';
            trocarCliente(); // Reseta card de perfil e inputs de nome
        }
        ...
        function enviarReservaAjax() {
            ...
            fetch('adicionar_reserva.php', { method: 'POST', body: fd })
                ...
                .then(res => {
                    ...
                    if (res.success) {
                        // Condicional: só mostra botão WhatsApp se houver link/telefone
                        let btnZapHtml = res.link_wpp ? `<button onclick="window.open('${res.link_wpp}', '_blank')" class="btn-ios-primary" style="background:#25D366; margin:0;">Confirmar via WhatsApp</button>` : '';

                        let modalHtml = `
            <div id="modalConfirmacaoManual" class="modal-overlay">
                <div class="modal-box">
                    <div class="material-icons" style="font-size: 60px; color: #28a745; margin-bottom: 15px;">check_circle</div>
                    ...
                    <div style="margin-top:25px; display:flex; flex-direction:column; gap:12px;">
                        ${btnZapHtml}
                        <button onclick="fecharModalEAtualizar()" class="btn-ios-secondary">Fechar e Nova Reserva</button>
                    </div>
                </div>
            </div>`;
        ...
        function verificarEEnviar() {
            ...
            // Ajuste na regra de validação: se semTel estiver marcado, ignora comprimento do tel
            if (!semTel && tel.length < 10) {
                alert('Por favor, insira um telefone válido ou marque a opção "Não possuo número".');
                return;
            }

            if (!data || !nome) { alert('Preencha os campos obrigatórios.'); return; }

            const btn = document.getElementById('btnSalvarManual');
            btn.disabled = true; btn.innerText = "Verificando...";

            // Se não tem telefone, pula checagem de duplicidade e envia direto
            if (semTel) {
                enviarReservaAjax();
                return;
            }
        ...
        function editarCliente() {
            document.getElementById('input-edit-nome').value = document.getElementById('card-nome').innerText;
        ...
        function importarWhatsParaFormulario() {
            ...
            linhas.forEach(linha => {
                const partes = linha.split(':'); if (partes.length < 2) return;
        ...
        async function analisarSalvarDireto() {
            const texto = document.getElementById('whats_dados').value.trim();
            if (!texto) return;
            const btn = event.target; btn.innerText = "Processando..."; btn.disabled = true;
            ...
            try {
                ...
                btn.innerText = "Salvar Direto"; btn.disabled = false;
                if (!res.success) return;
                let listaParaSalvar = [];
                for (let item of res.lista) {
                    if (!item.valido) continue;
                    if (item.duplicado) { if (confirm(`Duplicidade: ${item.dados.nome} já existe. Salvar assim mesmo?`)) listaParaSalvar.push(item.dados); }
        ...
        async function salvarListaFinal(lista) {
            ...
            try {
                ...
                let res = await req.json();
                if (res.success) {
                    let linksHtml = res.links.map(l => `<li>${l.nome}: <a href="${l.link}" target="_blank">WhatsApp</a></li>`).join('');
                    let modalHtml = `<div id="modalRelatorio" class="modal-overlay"><div class="modal-box"><h3>Processado!</h3><p>${res.salvos} reservas criadas.</p><ul style="text-align:left; max-height:200px; overflow:auto;">${linksHtml}</ul><button onclick="document.getElementById('modalRelatorio').remove(); fecharModalEAtualizar();" class="btn-ios-primary">Fechar</button></div></div>`;
                    document.body.insertAdjacentHTML('beforeend', modalHtml);
                }
            } catch (e) { }
        }
